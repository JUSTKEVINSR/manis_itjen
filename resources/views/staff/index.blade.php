                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($staffs as $staff)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $staff->nik }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $staff->jabatan }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $staff->departemen }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $staff->phone }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('staff.edit', $staff) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            <form action="{{ route('staff.destroy', $staff) }}" method="POST" class="inline-block ms-3">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No staff found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
